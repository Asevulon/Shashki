...
int Game::GetPossibleTurnsWHITEQ(int i, int j, vector<pair<int, int>>& out)
{
	...
	while ((I < 6) && (J > 1))
	{
		if ((board[I + 1][J - 1] == WHITE) || (board[I + 1][J - 1] == WHITEQ))break;

		if ((board[I + 1][J - 1] == BLACK) || (board[I + 1][J - 1] == BLACKQ))
		{
			while (board[I + 2][J - 2] == EMPTY)
			{
				out.push_back(ip(I + 2, J - 2));
				I++;
				J--;
				if ((I == 6) || (J == 1))break;
	...
	J = j;
	if ((I < 7) && (J > 0))
	while (board[I + 1][J - 1] == EMPTY)
	{
		out.push_back(ip(I + 1, J - 1));
		I++;
		J--;
			if ((I == 7) || (J == 0))break;
	}
...
int Game::GetPossibleTurnsBLACKQ(int i, int j, vector<pair<int, int>>& out)
{
	...
	while ((I < 6) && (J > 1))
	{
		if ((board[I + 1][J - 1] == BLACK) || (board[I + 1][J - 1] == BLACKQ))break;
